...
// STEP 2: CREATE MAP (globally accessible)
window.map3 = new mapboxgl.Map({
...
// STEP 3: CONTROLS
window.map3.addControl(new mapboxgl.NavigationControl(), 'top-right');
window.map3.addControl(new mapboxgl.FullscreenControl(), 'top-right');
window.map3.addControl(new mapboxgl.ScaleControl({ maxWidth: 80, unit: 'metric' }), 'bottom-left');

// STEP 4: ON MAP LOAD
window.map3.on('load', () => {
  console.log('Map 3 loaded successfully!');
  
  // FIX: Resize the map after it loads
  setTimeout(() => {
    window.map3.resize();
  }, 100);
  ...
  fetch('ridgewood_filtered.geojson')
    ...
    .then(data => {
      ...
      window.map3.addSource('ridgewood-buildings', {
        type: 'geojson',
        data: data
      });

      // STEP 6: ADD LAYER FOR BUILDINGS
      window.map3.addLayer({
        id: 'buildings-fill',
        type: 'fill',
      ...
      // STEP 8: ADD POPUPS
      window.map3.on('click', 'buildings-fill', (e) => {
      ...
      window.map3.on('mouseenter', 'buildings-fill', () => {
        window.map3.getCanvas().style.cursor = 'pointer';
      });
      window.map3.on('mouseleave', 'buildings-fill', () => {
        window.map3.getCanvas().style.cursor = '';
      });

      // STEP 9: BUTTONS
      document.getElementById('fitToData').addEventListener('click', () => {
        window.map3.fitBounds(bounds, { padding: 50, duration: 2000, maxZoom: 16 });
      });

      document.getElementById('resetFilters').addEventListener('click', () => {
        document.getElementById('searchFeature').value = '';
        window.map3.setFilter('buildings-fill', null);
      });
      ...
      document.getElementById('searchFeature').addEventListener('input', (e) => {
        ...
        if (!term) {
          ...
          window.map3.setFilter('buildings-fill', ['in', term, ['downcase', ['get', 'Address']]]);
        }
      });
